...
export async function POST(req: Request) {
  ...
  if (isGemini && !geminiApiKey) {
    return new Response(
      JSON.stringify({
        error: 'GOOGLE_GENAI_API_KEY is not set. Add it to your .env.local file to use Gemini models.',
      }),
  ...
  /** Pick the right provider model based on request */
  function getModel(modelId: string) {
    return isGemini ? gemini(modelId) : openai(modelId);
  }
  ...
  if (!isFollowUp) {
    try {
      const validationResult = await generateObject({
        model: getModel(isGemini ? GEMINI_MODELS.flash : "gpt-5.2"),
